	...
	factory: (require) => {
		...
		*/
		...
		function renderChip(tag) {
			...
			el.textContent = `⟳ 进度条 新版本 ${tag} 可用，点击更新`;
				...
				runUpdate(tag).then((result) => {
					...
					if (result.link) {
						navigator.clipboard?.writeText(updatePrompt(tag)).then(() => {
							el.textContent = `本地 link 安装：已跳过自动更新，更新提示词已复制到剪贴板`;
						}).catch(() => {
							el.textContent = `本地 link：请手动执行 pnpm add '${PACKAGE_SPEC}@github:${MIRROR}#${tag}'`;
						});
						el.setAttribute("title", "悬停查看本地 link 说明");
						return;
					}
					navigator.clipboard?.writeText(updatePrompt(tag)).then(() => {
						el.textContent = `自动更新失败（详见剪贴板提示词）：${result.detail.slice(0, 80)}`;
					}).catch(() => {
						el.textContent = `自动更新失败：${result.detail.slice(0, 80)}`;
					});
