...
dsh plugin --profile web add '@dsh-external/dsh-ui-progress@github:omdsh-dev/dsh-ui-progress#v0.9.5'
...
> 1. Run `dsh plugin --profile web add '@dsh-external/dsh-ui-progress@github:omdsh-dev/dsh-ui-progress#v0.9.5'` (the first run may fail because pnpm 11 blocks node-pty build scripts)
...
> On errors, first check the FAQ/known limitations in the README at https://github.com/omdsh-dev/dsh-ui-progress.
...
| `v0.9.5` (default) | `dsh-v0.1.2-alpha.1` | Current: migrated to the 0.1.2-alpha.1 client API (Chat legacy projection + ctx.slots.inject), with the compat self-diagnostic banner |
...
> Pinned tag via git dependency (public mirror, recommended): `pnpm add '@dsh-external/dsh-ui-progress@github:omdsh-dev/dsh-ui-progress#v0.9.5'` (or `github:omdsh-dev/dsh-ui-progress`; historical: 0809 users `#v0.9.0`, 0808 users `#v0.8.0`, 0807 users `#v0.6.0`, 0805 users `#v0.1.0`).
