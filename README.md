...
| `v0.9.0`（默认） | `snapshots/20260809T140917Z-a6bb5a95ba`（snapshot0809） | 新构建（原生 0809）：运行中新增**实时 token 生成速率**（估算 tok/s，首 token 到达起算，CJK 感知字符密度） |
...
- **实时 token 生成速率**（v0.9.0 新增）：运行中**模型正在生成**时（有流式 partial 内容、且无待处理的人机交互），进度条在已耗时旁显示实时速率（如 `12.3 tok/s`，斜体品牌色）。流式 chunk 不携带 token 计数（核心端只有回合结束后的 provider usage），因此该值是**估算值**：按 CJK 感知字符密度折算当前 partial 的 token 数（中日韩宽字符 1 字符 ≈ 1 token，其余按核心 token-meter 同款 4 字符 ≈ 1 token），除以**自首个可见 token 到达**起的解码耗时（排除 TTFT）——与核心端回合结束后显示的结算 tokens/s（outputTokens/decodeMs）口径一致，运行中数值可直接与结算值对比。每个新 step 重新起算；工具执行/等待人机交互/回合结束时不显示（结算后的精确速率由核心 StatsLine 呈现，避免重复展示同一事实）。
...
- 实时 token 速率为**估算值**（流式 chunk 无 token 计数）：CJK 感知字符密度是对真实分词的近似，推理/正文/工具参数一并计入；速率随窗口内最近一瞬的字符到达而波动，非 provider 报告值，回合结束后以核心 StatsLine 的结算 tokens/s 为准。
